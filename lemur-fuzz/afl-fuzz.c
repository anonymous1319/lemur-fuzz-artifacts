...
u32 fixed_count = 0;
u32 fix_send_req = 0;
u32 fix_success_rsp = 0;
u32 parser_through = 0;
u32 parser_success = 0;
u32 M2_start_region_ID = 0, M2_region_count = 0;

// Global variables for Semantic-aware Fuzzing
uint8_t output_buf[1024*1024];
u32 out_len = 0;
u32 pkt_num = 0; // packet num
void *packets; // packet
...
u32 nf_grammar_cal_succ = 0;
u32 nf_semantic_cal_succ = 0;
u32 nf_semantic_cal_total = 0;
...
static int g_sem_enabled = 1;    /* default semantic-aware mutator on */
static int skip_havoc = 0;  /* skip havoc stage */
static kliter_t(lms) *M2_begin_it = NULL;
static kliter_t(lms) *M2_end_it   = NULL;
...
static char  state_tmp_path[512];          
...
int send_over_network()
{
  ...
  if(strcmp(stage_short,"sem")==0 && strcmp(protocol_name,"MQTT")==0){ 
    ...
    semantic_total_sent = semantic_total_sent + M2_pkt_cnt;
  }
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    // calculate M2 region only
    // TODO: shm to notify M2 region
    // if (it == M2_begin_it) m2bit_set(1);
    // if (it == M2_end_it)   m2bit_set(0);
  ...
  if (M2_end_it == kl_end(kl_messages)) m2bit_set(0);
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    char asan_log_opt[512];
    snprintf(asan_log_opt, sizeof(asan_log_opt),
            "abort_on_error=1:"
            "detect_leaks=0:"
            "symbolize=1:"
            "allocator_may_return_null=1:"
            "log_path=%s/crashes/asan", out_dir);
    setenv("ASAN_OPTIONS", asan_log_opt, 1);
    fprintf(stderr, "[DEBUG] ASAN_OPTIONS=%s\n", getenv("ASAN_OPTIONS"));
    // char path[256];
    // snprintf(path, sizeof(path), "%s/crashes/asan", out_dir);
    // FILE* fp = fopen(path, "w");
    // if (!fp) perror("[DEBUG] Cannot write from main()");
    // else {
    //     fprintf(fp, "write from init forkserver\n");
    //     close(fp);
    // }
    ...
    fprintf(stderr, "[AFL DEBUG] Executing target: %s\n", target_path);
    for (char **arg = argv; *arg != NULL; arg++) {
        fprintf(stderr, "[AFL DEBUG] argv[] = %s\n", *arg);
    }
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (WIFSIGNALED(status) && !stop_soon) {
    ...
    char crash_info_path[512];
    snprintf(crash_info_path, sizeof(crash_info_path),
            "%s/crashes/crash_info_pid_%d.txt", out_dir, getpid()+2);

    FILE *log_fp = fopen(crash_info_path, "w");
    if (log_fp) {
        fprintf(log_fp, "Crash[%d] detected!\n", unique_crashes);
        fprintf(log_fp, "PID: %d\n", getpid()+2);
        fprintf(log_fp, "Kill Signal: %d (%s)\n", kill_signal, strsignal(kill_signal));
        if (queue_cur && queue_cur->fname) {
            fprintf(log_fp, "Input file: %s\n", queue_cur->fname);
        }

        char asan_log_path[512];
        snprintf(asan_log_path, sizeof(asan_log_path),
                "%s/crashes/asan.%d", out_dir, getpid()+2);

        FILE *asan_fp = fopen(asan_log_path, "r");
        if (asan_fp) {
            fprintf(log_fp, "\n===== ASAN STACK TRACE =====\n");
            char line[1024];
            while (fgets(line, sizeof(line), asan_fp)) {
                fputs(line, log_fp);
            }
            fclose(asan_fp);
        } else {
            fprintf(log_fp, "\n[WARNING] ASAN log file not found: %s\n", asan_log_path);
        }

        fclose(log_fp);
    } else {
        perror("Failed to write crash info log");
    }
...
keep_as_crash:
      ...
      u8* crash_log_fn = alloc_printf("%s/crash_logs/id:%06llu_sig:%02u.log",
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file,
        ...
        unique_hangs, max_depth, eps, M2_start_region_ID, M2_region_count,M2_cur_cnt, 
        (g_sem_total_ns / 1e9), 
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  // printf("inside common_fuzz_stuff, len: %u\n", len);
  ...
  /* new M2 region is [new_begin, M2_next) */
  // if (M2_prev == NULL) {
  //   M2_begin_it = kl_begin(kl_messages);
  // } else {
  //   M2_begin_it = kl_next(M2_prev);
  // }

  // if(M2_next == NULL){
  //   M2_end_it = kl_end(kl_messages);
  // }
  // else{
  //   M2_end_it = M2_next; 
  // }
...
parser_through++;
packets = generate_packets_by_protocol(protocol_name, PKTS_MAX);
if(packets == NULL) {
  goto real_havoc_stage;
}
if(strcmp(protocol_name, "MQTT") == 0) {
  pkt_num = parse_mqtt_msg(out_buf, len, (mqtt_packet_t*)packets, PKTS_MAX);
}
else if(strcmp(protocol_name, "RTSP") == 0){
  pkt_num = parse_rtsp_msg(out_buf, len, (rtsp_packet_t*)packets, PKTS_MAX);
}
else if(strcmp(protocol_name, "FTP") == 0){
  pkt_num = parse_ftp_msg(out_buf, len, (ftp_packet_t*)packets, PKTS_MAX);
}
else if(strcmp(protocol_name, "SMTP") == 0){
  pkt_num = parse_smtp_msg(out_buf, len, (smtp_packet_t*)packets, PKTS_MAX);
}
else if(strcmp(protocol_name, "SIP") == 0){
  pkt_num = parse_sip_msg(out_buf, len, (sip_packet_t*)packets, PKTS_MAX);
}
else if(strcmp(protocol_name, "DTLS12") == 0){
  pkt_num = parse_dtls_msg(out_buf, len, (dtls_packet_t*)packets, PKTS_MAX);
}
else{
  free(packets);
  goto real_havoc_stage;
...
for(stage_cur = 0; stage_cur < stage_max; stage_cur++) {
  memset(packets, 0, pkt_elem_size(protocol_name) * PKTS_MAX);
  ...
  /* When SEM_OFF=1, the semantic-aware mutator stage is skipped, but the
   * constraint fixer below still runs — this is the "LEMUR-Fuzz Fix" ablation
   * variant (fixer-only) from the paper's RQ3. */
...
stage_finds[STAGE_SEMANTIC]  += new_hit_cnt - orig_hit_cnt;
stage_cycles[STAGE_SEMANTIC] += stage_max;
...
int main(int argc, char** argv) {
  ...
  {
    g_sem_enabled = 0;
    OKF("Semantic-aware mutator off (ablation: fixer-only)");
  }
  else{
    OKF("Semantic-aware mutator on");
  }
    if (getenv("HAVOC_OFF"))
  {
    skip_havoc = 1;
