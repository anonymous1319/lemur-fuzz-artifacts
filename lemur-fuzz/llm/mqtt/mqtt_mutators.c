...
void mutate_connect_flags(mqtt_connect_packet_t* pkts, int num_pkts) {
    ...
    for (int i = 0; i < num_pkts; i++) {
        ...
        switch (mut_type) {
        /* bitfield connect_flags: dimension breakdown A..H */
        /* ===== A. Canonical form ===== */
        case 0:
            {
            ...
            break;
        /* ===== B. Boundary values: n/a for connect_flags (single byte, no length) ===== */
        /* ===== C. Equivalence-class alternatives: n/a for connect_flags (combinatorial bitfield) ===== */
        ...
        case 6:
            mutated = ((original >> 1) | (original << 7)) & 0xFF;
            break;
        /* ===== E. Encoding-validity preservation: n/a for connect_flags (raw byte) ===== */
        /* ===== F. Padding / alignment: n/a for connect_flags (single byte) ===== */
        /* ===== G. Prefix / suffix variants: n/a for connect_flags (single byte) ===== */
        /* ===== H. Random valid mix: covered by case 0 (random legal bitfield combo) ===== */
        }
    ...
    for (int i = 0; i < num_pkts; i++) {
        ...
        switch (strategy) {
            ...
            case 5:
                mutated = orig - (rand() % 1000);
                break;
            /* ===== C. Equivalence-class alternatives: n/a for keep_alive (pure numeric) ===== */
            /* ===== D. Allowed bitfield / enum / range ===== */
            case 3:
                mutated = rand() % 10000;
                break;
            /* ===== E. Encoding-validity preservation: n/a for keep_alive (uint16) ===== */
            /* ===== F. Padding / alignment: n/a for keep_alive (uint16) ===== */
            /* ===== G. Prefix / suffix variants: n/a for keep_alive (uint16) ===== */
            /* ===== H. Random valid mix ===== */
            case 6:
                mutated = rand();
                break;
...
void mutate_connect_property_len(mqtt_connect_packet_t *packets, int num_packets) {
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        switch (mode) {
            /* ===== A. Canonical form: n/a for property_len (pure numeric length) ===== */
            /* ===== B. Boundary values ===== */
            case 0:
                pkt->variable_header.property_len = 0;
                break;
            ...
            case 3:
                ...
                break;
            /* ===== C. Equivalence-class alternatives: n/a for property_len (numeric) ===== */
            /* ===== D. Allowed bitfield / enum / range: n/a for property_len (numeric) ===== */
            /* ===== E. Encoding-validity preservation: n/a for property_len (numeric) ===== */
            /* ===== F. Padding / alignment: n/a for property_len (length field, fixed-size uint32) ===== */
            /* ===== G. Prefix / suffix variants: n/a for property_len (numeric) ===== */
            /* ===== H. Random valid mix ===== */
            case 1:
...
void mutate_connect_properties(mqtt_connect_packet_t *packets, int num_packets) {
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        for (int n = 0; n < num_props; ++n) {
            int pick = rand() % 9;
            switch (pick) {
                /* ===== A. Canonical form: typical property values for CONNECT props ===== */
                ...
                /* ===== B. Boundary values: n/a for properties (each property encodes its own value range) ===== */
                ...
                /* ===== D. Allowed bitfield / enum / range: covered by fixed PID_* identifiers above ===== */
                ...
                /* ===== F. Padding / alignment: n/a for properties (variable-length records) ===== */
                /* ===== G. Prefix / suffix variants: n/a for properties (key/value records) ===== */
                /* ===== H. Random valid mix ===== */
...
void mutate_connect_client_id(mqtt_connect_packet_t *packets, int num_packets) {
    for (int i = 0; i < num_packets; ++i) {
        ...
        int weights[9] = {0, 70, 0, 0, 0, 0, 0, 0, 0};
        int mut_type = pick_weighted(weights, 9);
        ...
        switch (mut_type) {
            ...
            /* ===== F. Padding / alignment ===== */
            case 8: {
                int len = 1 + rand() % (MAX_CLIENT_ID_LEN - 2);
                for (int j = 0; j < len; ++j) {
                    cid[j] = valid_chars[rand() % (sizeof(valid_chars) - 1)];
                }
                int pad = 4 + rand() % 8;
                for (int j = len; j < len + pad && j < MAX_CLIENT_ID_LEN - 1; ++j) {
                    cid[j] = ' ';
                }
                cid[(len + pad < MAX_CLIENT_ID_LEN) ? (len + pad) : (MAX_CLIENT_ID_LEN - 1)] = '\0';
                break;
            }
...
void mutate_connect_will_property_len(mqtt_connect_packet_t *packets, int num_packets) {
    for (int i = 0; i < num_packets; ++i) {
        ...
        switch (strategy) {
            /* ===== A. Canonical form: n/a for will_property_len (numeric length) ===== */
            /* ===== B. Boundary values ===== */
            case 4:
                packets[i].payload.will_property_len = 0;
                memset(packets[i].payload.will_properties, 0, MAX_PROPERTIES_LEN);
            ...
            case 2:
                ...
                break;
            /* ===== D. Allowed bitfield / enum / range: n/a for will_property_len (numeric length) ===== */
            /* ===== E. Encoding-validity preservation: n/a for will_property_len (numeric length) ===== */
            /* ===== F. Padding / alignment ===== */
            case 3:
                ...
                break;
            /* ===== G. Prefix / suffix variants: n/a for will_property_len (numeric length) ===== */
            /* ===== H. Random valid mix ===== */
            case 0:
...
void mutate_connect_will_properties(mqtt_connect_packet_t *packets, int num_packets) {
    for (int i = 0; i < num_packets; ++i) {
        ...
        switch (strategy) {
            ...
            /* ===== H. Random valid mix: covered by case 3 (MAX random bytes) ===== */
            default:
                break;
        }
    }
...
void mutate_connect_will_topic(mqtt_connect_packet_t *packets, int num_packets) {
    /* Canonical, specification-recommended topic names (slash-separated paths). */
    ...
    /* Legal UTF-8 characters for an MQTT topic name (visible ASCII + spaces). */
    const char valid_chars[] = "abcdefghijklmnopqrstuvwxyz/+-_ 0123456789";

    /* Legitimate multi-byte UTF-8 topic-name fragments (valid, rare-but-legal). */
    const char *utf8_frags[] = {
        "\xE4\xBC\xA0\xE6\x84\x9F\xE5\x99\xA8",  /* 传感器 */
        "\xE6\xB8\xA9\xE5\xBA\xA6",              /* 温度 */
        "\xCE\xB1\xCE\xB2\xCE\xB3",              /* αβγ */
    };
    int total_utf8 = sizeof(utf8_frags) / sizeof(utf8_frags[0]);

    /* Specification-allowed prefix/suffix variants (reserved prefix $SYS/, suffix tags). */
    const char *prefixes[] = { "$SYS/", "app/", "" };
    const char *suffixes[] = { "/debug", "/status", "" };

    for (int i = 0; i < num_packets; ++i) {

        if (!(packets[i].variable_header.connect_flags & 0x04)) continue;  // WillFlag=1

        int weights[7] = {0, 50, 0, 0, 0, 0, 0};
        int strategy = pick_weighted(weights, 7);
        ...
        switch (strategy) {
            ...
            /* ===== B. Boundary values ===== */
            case 5: {
                topic[0] = '\0';                       /* empty string (min length) */
                break;
            }
            /* ===== C. Equivalence-class alternatives ===== */
            /* Semantically valid but structurally distinct topic shapes:
             * single-level vs multi-level, slash-separated paths, wildcards.
             * (No bitflips — those are general-purpose havoc, not semantic.) */
            case 4: {
                const char *shapes[] = { "a", "a/b", "a/b/c", "a/b/c/d", "+/#", "#" };
                int n_shapes = sizeof(shapes) / sizeof(shapes[0]);
                strncpy(topic, shapes[rand() % n_shapes], MAX_TOPIC_LEN - 1);
                topic[MAX_TOPIC_LEN - 1] = '\0';
                break;
            }
            /* ===== D. Allowed bitfield / enum / range: n/a for will_topic (string) ===== */
            /* ===== E. Encoding-validity preservation ===== */
            /* will_topic MUST be valid UTF-8: emit a legal, mostly-ASCII topic that
             * embeds a valid multi-byte UTF-8 fragment (rare-but-legal). */
            case 2: {
                int pos = 0;
                pos += snprintf(topic + pos, MAX_TOPIC_LEN - pos, "sensors/");
                const char *frag = utf8_frags[rand() % total_utf8];
                pos += snprintf(topic + pos, MAX_TOPIC_LEN - pos, "%s", frag);
                pos += snprintf(topic + pos, MAX_TOPIC_LEN - pos, "/value");
                if (pos >= MAX_TOPIC_LEN) pos = MAX_TOPIC_LEN - 1;
                topic[pos] = '\0';
                break;
            }
            /* ===== F. Padding / alignment ===== */
            /* Pad a canonical topic to a fixed length WITHOUT altering semantics:
             * trailing legal UTF-8 characters (space/slash), preserving validity. */
            case 3: {
                const char *prefix = base_topics[rand() % total_base];
                strncpy(topic, prefix, MAX_TOPIC_LEN - 1);
                topic[MAX_TOPIC_LEN - 1] = '\0';
                size_t len = strlen(topic);
                while (len + 1 < MAX_TOPIC_LEN) {
                    topic[len++] = (rand() & 1) ? ' ' : '/';
                }
                topic[MAX_TOPIC_LEN - 1] = '\0';
                break;
            ...
            /* ===== G. Prefix / suffix variants ===== */
            /* Specification-allowed extensions: reserved prefix $SYS/, suffix tags. */
            case 6: {
                const char *pfx = prefixes[rand() % 3];
                const char *core = base_topics[rand() % total_base];
                const char *sfx = suffixes[rand() % 3];
                snprintf(topic, MAX_TOPIC_LEN, "%s%s%s", pfx, core, sfx);
                break;
            }
            /* ===== H. Random valid mix ===== */
            case 1: {
...
void mutate_connect_will_payload(mqtt_connect_packet_t *packets, int num_packets) {
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        switch (strategy) {
            ...
            /* ===== D. Allowed bitfield / enum / range: n/a for will_payload (binary blob) ===== */
            /* ===== E. Encoding-validity preservation ===== */
            case 5: {
            ...
            /* ===== G. Prefix / suffix variants: n/a for will_payload (binary blob) ===== */
            /* ===== H. Random valid mix ===== */
            case 1: {
                int l = 1 + rand() % 64;
                for (int j = 0; j < l; ++j) {
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        switch (strategy) {
            ...
            case 1:
                ...
                break;
            /* ===== C. Equivalence-class alternatives: n/a for will_payload_len (numeric length) ===== */
            /* ===== D. Allowed bitfield / enum / range: n/a for will_payload_len (numeric length) ===== */
            /* ===== E. Encoding-validity preservation: n/a for will_payload_len (numeric length) ===== */
            /* ===== F. Padding / alignment: n/a for will_payload_len (numeric length) ===== */
            /* ===== G. Prefix / suffix variants: n/a for will_payload_len (numeric length) ===== */
            /* ===== H. Random valid mix ===== */
            case 4:
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        switch (strategy) {
            ...
            case 7: {
                ...
                strncpy(pkt->payload.user_name, pfx, MAX_CLIENT_ID_LEN - 1);
                int pl = (int)strlen(pkt->payload.user_name);
                int l = 3 + rand() % 8;
                for (int j = 0; j < l && pl + j < MAX_CLIENT_ID_LEN - 1; ++j) {
                    pkt->payload.user_name[pl + j] = valid_chars[rand() % (sizeof(valid_chars) - 1)];
                }
                pkt->payload.user_name[(pl + l < MAX_CLIENT_ID_LEN) ? (pl + l) : (MAX_CLIENT_ID_LEN - 1)] = '\0';
                break;
            }
            /* ===== H. Random valid mix ===== */
            case 5: {
...
void mutate_connect_password(mqtt_connect_packet_t *packets, int num_packets) {
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        int weights[9] = {70, 0, 0, 0, 0, 0, 0, 0, 0};
        int strategy = pick_weighted(weights, 9);
        ...
        switch (strategy) {
            ...
            /* ===== C. Equivalence-class alternatives: n/a for password (binary blob) ===== */
            /* ===== D. Allowed bitfield / enum / range ===== */
            case 7: {
            ...
            case 8: {
                ...
                for (int j = 0; j < (int)sl && l + j < MAX_CLIENT_ID_LEN; ++j) {
                    pkt->payload.password[l + j] = sfx[j];
                }
                pkt->payload.password_len = (l + sl < MAX_CLIENT_ID_LEN) ? (uint16_t)(l + sl) : (uint16_t)MAX_CLIENT_ID_LEN;
                break;
            }
            /* ===== H. Random valid mix ===== */
            case 3: {
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        switch (strategy) {
            ...
            case 5:
                ...
                break;
            /* ===== C. Equivalence-class alternatives: n/a for password_len (numeric length) ===== */
            /* ===== D. Allowed bitfield / enum / range: n/a for password_len (numeric length) ===== */
            /* ===== E. Encoding-validity preservation: n/a for password_len (numeric length) ===== */
            /* ===== F. Padding / alignment: n/a for password_len (numeric length) ===== */
            /* ===== G. Prefix / suffix variants: n/a for password_len (numeric length) ===== */
            /* ===== H. Random valid mix ===== */
            case 4:
...
void mutate_subscribe_packet_identifier(mqtt_subscribe_packet_t *subs, size_t num_subs) {
    for (size_t i = 0; i < num_subs; ++i) {
        ...
        switch (strategy) {
            /* ===== A. Canonical form: n/a for packet_identifier (pure numeric id) ===== */
            /* ===== B. Boundary values ===== */
            case 0:
                mutated = 0;
                break;
            ...
            case 5:
                ...
                break;
            /* ===== E. Encoding-validity preservation: n/a for packet_identifier (uint16) ===== */
            /* ===== F. Padding / alignment: n/a for packet_identifier (uint16) ===== */
            /* ===== G. Prefix / suffix variants: n/a for packet_identifier (uint16) ===== */
            /* ===== H. Random valid mix ===== */
            case 3:
...
void mutate_subscribe_properties(mqtt_subscribe_packet_t *subs, size_t num_subs) {
    ...
    for (size_t i = 0; i < num_subs; ++i) {
        ...
        /* ===== A. Canonical form: Subscription Identifier (typical property) ===== */
        if (rand() % 2) {
            uint8_t tmp[4];
        ...
        /* ===== B. Boundary values: n/a for subscribe_properties (SID range 1..16383 only) ===== */
        /* ===== C. Equivalence-class alternatives: n/a for subscribe_properties (SID + User Property only) ===== */
        /* ===== D. Allowed bitfield / enum / range: covered by fixed IDs 0x0B and 0x26 ===== */
        /* ===== E. Encoding-validity preservation: n/a for subscribe_properties (ASCII literals) ===== */
        /* ===== F. Padding / alignment: n/a for subscribe_properties (variable-length records) ===== */
        /* ===== G. Prefix / suffix variants: n/a for subscribe_properties (key/value records) ===== */
        /* ===== H. Random valid mix: User Property pairs with random key/value selection ===== */
        static const char *keys[] = {"source", "priority", "note", "device"};
        static const char *vals[] = {"sensor1", "high", "ok", "edge"};
        int upn = rand() % 4; /* 0..3 */
        for (int t = 0; t < upn; ++t) {
...
void mutate_subscribe_topic_filter(mqtt_subscribe_packet_t *subs, size_t num_subs) {
    ...
    for (size_t i = 0; i < num_subs; ++i) {
        ...
        for (int j = 0; j < pkt->payload.topic_count; ++j) {
            ...
            int weights[7] = {20, 20, 20, 20, 10, 10, 10};
            int strategy = pick_weighted(weights, 7);
            ...
            switch (strategy) {
                ...
                /* ===== D. Allowed bitfield / enum / range: n/a for topic_filter (string) ===== */
                /* ===== E. Encoding-validity preservation: n/a for topic_filter (ASCII charset used) ===== */
                /* ===== F. Padding / alignment ===== */
                case 6: {
                    int levels = 1 + rand() % 3;
                    for (int l = 0; l < levels; ++l) {
                    ...
                    while (pos < MAX_TOPIC_LEN - 1) { APPEND_CHAR(' '); }
                    break;
                }
                /* ===== G. Prefix / suffix variants ===== */
                case 3: {
        ...
        for (int j = 0; j < pkt->payload.topic_count; ++j) {
            ...
            switch (strategy) {
                ...
                case 7:
                    *qos ^= (1 << (rand() % 3));
                    break;
                /* ===== E. Encoding-validity preservation: n/a for qos (uint8 enum) ===== */
                /* ===== F. Padding / alignment: n/a for qos (single byte) ===== */
                /* ===== G. Prefix / suffix variants: n/a for qos (single byte) ===== */
                /* ===== H. Random valid mix ===== */
                case 5:
                    *qos = rand() % 3;
                    break;
...
void mutate_subscribe_topic_count(mqtt_subscribe_packet_t *subs, size_t num_subs) {
    for (size_t i = 0; i < num_subs; ++i) {
        ...
        int strategy = pick_weighted(weights, 10);
        switch (strategy) {
            /* ===== A. Canonical form: n/a for topic_count (pure numeric count) ===== */
            /* ===== B. Boundary values ===== */
            case 0:
                *count = 0;
                break;
            ...
            case 8:
                *count ^= 0x01;
                break;
            /* ===== E. Encoding-validity preservation: n/a for topic_count (uint8) ===== */
            /* ===== F. Padding / alignment: n/a for topic_count (single byte) ===== */
            /* ===== G. Prefix / suffix variants: n/a for topic_count (single byte) ===== */
            /* ===== H. Random valid mix ===== */
            case 4:
...
void mutate_publish_topic_name(mqtt_publish_packet_t *pkts, size_t num) {
    for (size_t i = 0; i < num; ++i) {
        ...
        int weights[11] = { 0, 0, 0, 0, 0, 0, 0, 50, 0, 0, 10};
        int strategy = pick_weighted(weights, 11);
        switch (strategy) {
            /* ===== A. Canonical form ===== */
            case 7:
            ...
            case 10:
                strcpy(topic, "a/b");
                for (int j = 4; j < MAX_TOPIC_LEN - 1; j++) topic[j] = ' ';
                topic[MAX_TOPIC_LEN - 1] = '\0';
                break;
            /* ===== G. Prefix / suffix variants ===== */
            case 9:
...
void mutate_publish_packet_identifier(mqtt_publish_packet_t *pkts, size_t num) {
    for (size_t i = 0; i < num; ++i) {
        ...
        switch (strategy) {
            ...
            /* ===== C. Equivalence-class alternatives: n/a for packet_identifier (single id) ===== */
            /* ===== D. Allowed bitfield / enum / range ===== */
...
void mutate_publish_properties(mqtt_publish_packet_t *pkts, size_t num) {
    ...
    for (size_t i = 0; i < num; ++i) {
        ...
        switch (strategy) {
            ...
            case 0:
                break;
            /* ===== C. Equivalence-class alternatives: covered by distinct property-ID cases above ===== */
            /* ===== D. Allowed bitfield / enum / range: covered by fixed PID_* identifiers ===== */
            /* ===== E. Encoding-validity preservation ===== */
            case 4:
            {
        ...
        switch (strategy) {
            ...
            case 8:
                *len = strlen(pkt->variable_header.topic_name);
                ...
                break;
            /* ===== D. Allowed bitfield / enum / range: n/a for payload (binary blob) ===== */
            /* ===== E. Encoding-validity preservation ===== */
            case 7:
                p[0] = 0xC0; p[1] = 0x00;
                *len = 2;
...
void mutate_publish_qos(mqtt_publish_packet_t *pkts, size_t num) {
    for (size_t i = 0; i < num; ++i) {
        ...
        switch (strategy) {
            ...
            /* ===== C. Equivalence-class alternatives: n/a for qos (single byte enum) ===== */
            /* ===== D. Allowed bitfield / enum / range ===== */
            case 3: *qos = 3; break;
            case 6: *qos = (*qos + 1) % 4; break;
            case 7: *qos = 0xFF & ~(*qos); break;
            case 9: *qos = 0xAA; break;
            /* ===== E. Encoding-validity preservation: n/a for qos (uint8) ===== */
            /* ===== F. Padding / alignment: n/a for qos (single byte) ===== */
            /* ===== G. Prefix / suffix variants: n/a for qos (single byte) ===== */
            /* ===== H. Random valid mix ===== */
...
void mutate_publish_dup(mqtt_publish_packet_t *pkts, size_t num) {
    for (size_t i = 0; i < num; ++i) {
        ...
        switch (strategy) {
            ...
            /* ===== C. Equivalence-class alternatives: n/a for dup (single bit flag) ===== */
            /* ===== D. Allowed bitfield / enum / range ===== */
            ...
            /* ===== E. Encoding-validity preservation: n/a for dup (uint8) ===== */
            /* ===== F. Padding / alignment: n/a for dup (single byte) ===== */
            /* ===== G. Prefix / suffix variants: n/a for dup (single byte) ===== */
            /* ===== H. Random valid mix ===== */
...
void mutate_publish_retain(mqtt_publish_packet_t *pkts, size_t num) {
    for (size_t i = 0; i < num; ++i) {
        ...
        switch (strategy) {
            ...
            /* ===== E. Encoding-validity preservation: n/a for retain (uint8) ===== */
            /* ===== F. Padding / alignment: n/a for retain (single byte) ===== */
            /* ===== G. Prefix / suffix variants: n/a for retain (single byte) ===== */
            /* ===== H. Random valid mix ===== */
...
void mutate_unsubscribe_properties(mqtt_unsubscribe_packet_t *pkts, int num) {
    ...
    for (int i = 0; i < num; ++i) {
        ...
        switch (strategy) {
            ...
            /* ===== D. Allowed bitfield / enum / range: n/a for properties (User-Property-only set) ===== */
            /* ===== E. Encoding-validity preservation: n/a for properties (fixed ASCII literals) ===== */
            /* ===== F. Padding / alignment: n/a for properties (variable-length records) ===== */
            /* ===== G. Prefix / suffix variants: n/a for properties (key/value records) ===== */
            /* ===== H. Random valid mix: covered by random key/value selection above ===== */
        }
    ...
    for (int i = 0; i < num; ++i) {
        ...
        switch (strategy) {
            ...
            case 2:
                ...
                break;
            /* ===== G. Prefix / suffix variants: n/a for topic_filters (covered by case 4 /# suffix) ===== */
            /* ===== H. Random valid mix ===== */
            case 6:
                if (count > 0) {
                    int idx = rand() % count;
...
void mutate_auth_reason_code(mqtt_auth_packet_t *pkts, int num) {
    for (int i = 0; i < num; i++) {
        ...
        switch (strategy) {
            ...
            /* ===== C. Equivalence-class alternatives: n/a for reason_code (spec enum members only) ===== */
            /* ===== D. Allowed bitfield / enum / range ===== */
            case 9: *rc = 0x10; break;
            /* ===== E. Encoding-validity preservation: n/a for reason_code (numeric byte) ===== */
            /* ===== F. Padding / alignment: n/a for reason_code (single byte) ===== */
            /* ===== G. Prefix / suffix variants: n/a for reason_code (single byte) ===== */
            /* ===== H. Random valid mix ===== */
...
void mutate_auth_properties(mqtt_auth_packet_t *pkts, int num) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        switch (strategy) {
            ...
            /* ===== D. Allowed bitfield / enum / range: covered by fixed PID_* identifiers (0x15/0x16/0x1F/0x26) ===== */
            /* ===== E. Encoding-validity preservation: n/a for properties (ASCII literals, random binary data) ===== */
            /* ===== F. Padding / alignment: n/a for properties (variable-length records) ===== */
            /* ===== G. Prefix / suffix variants: n/a for properties (key/value records) ===== */
            /* ===== H. Random valid mix ===== */
            case 5:
