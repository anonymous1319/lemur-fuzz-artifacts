...
/* F. Padding / alignment — fill media_type to capacity with a fixed byte */
static void op_media_type_padding(sip_accept_hdr_t *h) {
    smemset(h->media_type, sizeof h->media_type, ' ', sizeof h->media_type - 1);
    h->slash = '/';
    if (h->sub_type[0] == '\0') scpy(h->sub_type, sizeof h->sub_type, "sdp");
}

/* G. Prefix / suffix variants — inject CRLF / header split into media_type */
static void op_media_type_prefix_suffix(sip_accept_hdr_t *h) {
    /* reuse the existing CRLF-injection value generator (defined below) */
...
static void mutate_one_media_type(sip_accept_hdr_t *h, unsigned *rs) {
    ...
    /* A..H ordered table; IDs: 0=A, 7=C, 2=C, 6=E, 8=F, 9=G, 10=H */
    static const unsigned legal_ops[] = {0, 7, 2, 6, 8, 9, 10};
    ...
    /* ===== B. Boundary values: n/a for token string field ===== */

    /* ===== C. Equivalence-class alternatives ===== */
    case 7:  op_vendor_x(h, rs);         break;
    case 2:  op_set_star_star(h);        break;

    /* ===== D. Allowed bitfield / enum / range: n/a for free-form token ===== */

    /* ===== E. Encoding-validity preservation ===== */
    case 6:  op_toggle_case(h);          break;

    /* ===== F. Padding / alignment ===== */
    case 8:  op_media_type_padding(h);    break;

    /* ===== G. Prefix / suffix variants ===== */
    case 9:  op_media_type_prefix_suffix(h); break;

    /* ===== H. Random valid mix ===== */
    case 10: op_garbage_token(h, rs);     break;
...
/* F. Padding / alignment — fill sub_type to capacity */
static void st_padding(sip_accept_hdr_t *h) {
    sfill(h->sub_type, sizeof h->sub_type, ' ', sizeof h->sub_type - 1);
    if (!h->slash) h->slash = '/';
}

/* G. Prefix / suffix variants — leading slash / CRLF injection */
static void st_prefix_suffix(sip_accept_hdr_t *h) {
    st_crlf_inject(h);
}

/* H. Random valid mix — random alnum sub_type built from the legal token charset */
static void st_random_mix(sip_accept_hdr_t *h, unsigned *rs) {
    gen_token(h->sub_type, sizeof h->sub_type, rs, 4, (size_t)(sizeof h->sub_type - 1));
    if (!h->slash) h->slash = '/';
    if (!h->media_type[0]) scpy(h->media_type, sizeof h->media_type, "application");
}
...
static void mutate_one_sub_type(sip_accept_hdr_t *h, unsigned *rs) {
    ...
    /* A..H ordered table; IDs: 0=A, 1=C, 6=C, 5=E, 7=F, 8=G, 9=H */
    static const unsigned legal_ops[] = {0, 1, 6, 5, 7, 8, 9};
    ...
    /* ===== F. Padding / alignment ===== */
    case 7:  st_padding(h);              break;

    /* ===== G. Prefix / suffix variants ===== */
    case 8:  st_prefix_suffix(h);         break;

    /* ===== H. Random valid mix ===== */
    case 9:  st_random_mix(h, rs);        break;
...
/* F. Padding / alignment — fill params buffer to capacity */
static void ps_padding(sip_accept_hdr_t *h) { ps_very_long(h); }

/* G. Prefix / suffix variants — CRLF / header injection in params */
static void ps_prefix_suffix(sip_accept_hdr_t *h) { ps_header_inject(h); }

/* H. Random valid mix — random alnum key=value pairs from the legal charset */
static void ps_random_mix(sip_accept_hdr_t *h, unsigned *rs) {
    size_t cap = sizeof h->params;
    if (cap < 8) { ps_set_valid_basic(h); return; }
    size_t pos = 0;
    while (pos + 10 < cap - 1) {
        char k[8]; gen_token(k, sizeof k, rs, 2, 5);
        char v[8]; gen_token(v, sizeof v, rs, 2, 5);
        int n = snprintf(h->params + pos, cap - pos, ";%s=%s", k, v);
...
static void mutate_one_params(sip_accept_hdr_t *h, unsigned *rs) {
    ...
    /* A..H ordered table; IDs: 0=A, 8=C, 10=E, 11=F, 12=G, 13=H */
    static const unsigned legal_ops[] = {0, 8, 10, 11, 12, 13};
    unsigned op = legal_ops[
        rnd_pick(rs, (unsigned)(sizeof(legal_ops)/sizeof(legal_ops[0])))
    ];
    ...
    /* ===== B. Boundary values: n/a for params list field ===== */

    /* ===== C. Equivalence-class alternatives ===== */
    case 8:  ps_duplicate_keys(h);   break;

    /* ===== D. Allowed bitfield / enum / range: n/a for free-form params ===== */

    /* ===== E. Encoding-validity preservation ===== */
    case 10: ps_quoted_string(h);    break;

    /* ===== F. Padding / alignment ===== */
    case 11: ps_padding(h);          break;

    /* ===== G. Prefix / suffix variants ===== */
    case 12: ps_prefix_suffix(h);    break;

    /* ===== H. Random valid mix ===== */
    case 13: ps_random_mix(h, rs);   break;
...
static void mutate_one_date_value(sip_date_hdr_t *h, unsigned *rs) {
    ...
    /* A..H ordered table; IDs: 0=A, 1=B, 2=B, 19=C, 20=C, 21=E, 22=F */
    static const unsigned legal_ops[] = {0, 1, 2, 19, 20, 21, 22};
    unsigned op = legal_ops[
        rnd_pick(rs, (unsigned)(sizeof(legal_ops)/sizeof(legal_ops[0])))
    ];
    ...
    switch (op) {
      ...
      /* ===== B. Boundary values (temporal: past / future extremes) ===== */
      case 1:  dt_fixed_past(h);     break;  /* IMF-fixdate */
      case 2:  dt_fixed_future(h);   break;  /* IMF-fixdate */

      /* ===== C. Equivalence-class alternatives (alternate legal date formats) ===== */
      case 19: dt_rfc850(h);         break;  /* RFC850-date */
      case 20: dt_ansi_ctime(h);     break;  /* asctime-date */

      /* ===== D. Allowed bitfield / enum / range: n/a for date string ===== */

      /* ===== E. Encoding-validity preservation (case-variant but still legal IMF-fixdate) ===== */
      case 21: dt_lowercase(h);      break;

      /* ===== F. Padding / alignment (fill rfc1123 buffer to capacity) ===== */
      case 22: dt_maxlen(h);         break;

      /* ===== G. Prefix / suffix variants: n/a for strict date format ===== */

      /* ===== H. Random valid mix: n/a for strict date format ===== */
...
/* G. Prefix / suffix variants — CRLF injection into scheme token */
static void op_enc_prefix_suffix(sip_encryption_hdr_t *h){
    op_scheme_set(h, "pgp\r\nVia: SIP/2.0/UDP evil");
}
/* H. Random valid mix — random alnum scheme token from the legal charset */
static void op_enc_random_mix(sip_encryption_hdr_t *h, unsigned *rs){
    gen_token(h->scheme, sizeof h->scheme, rs, 3, (size_t)(sizeof h->scheme - 1));
}

static void mutate_one_enc(sip_encryption_hdr_t *h, unsigned *rs) {
    if (h->scheme[0] == '\0') op_scheme_set(h, "pgp");

    /* A..H ordered table; IDs: 0=A, 8=B, 9=C, 11=C, 1=D, 2=D, 3=D, 10=E, 15=E, 7=F, 16=G, 17=H */
    static const unsigned legal_ops[] = {0, 8, 9, 11, 1, 2, 3, 10, 15, 7, 16, 17};
    unsigned op = legal_ops[
        rnd_pick(rs, (unsigned)(sizeof(legal_ops)/sizeof(legal_ops[0])))
    ];
...
static void mutate_one_expires(sip_expires_hdr_t *h, unsigned *rs) {
    /* A..H ordered table; IDs: 3=A, 0=B, 1=B, 2=B, 5=B, 6=B, 10=C, 11=C */
    static const unsigned legal_ops[] = {3, 0, 1, 2, 5, 6, 10, 11};
    unsigned op = legal_ops[
        rnd_pick(rs, (unsigned)(sizeof(legal_ops)/sizeof(legal_ops[0])))
    ];
    ...
    switch (op) {
      ...
      /* ===== B. Boundary values (0, 1, typical, uint32-max, over-max) ===== */
      ...
      /* ===== C. Equivalence-class alternatives (date-form alternatives) ===== */
      case 10: set_val(h, "Sat, 13 Nov 2010 23:29:00 GMT"); break;
      case 11: set_val(h, "Mon, 01 Jan 2035 00:00:00 GMT"); break;

      /* ===== D. Allowed bitfield / enum / range: n/a for numeric expires ===== */

      /* ===== E. Encoding-validity preservation: n/a for pure numeric ===== */

      /* ===== F. Padding / alignment: n/a for pure numeric ===== */

      /* ===== G. Prefix / suffix variants: n/a for pure numeric ===== */

      /* ===== H. Random valid mix: n/a for pure numeric ===== */
...
static void mutate_one_from(sip_from_hdr_t *h, unsigned *rs){
    ...
    /* A..H ordered table; IDs: 0=A, 1=A, 2=B, 5=B, 6=C, 7=C, 8=C, 10=C, 4=C, 16=F */
    static const unsigned legal_ops[] = {0, 1, 2, 5, 6, 7, 8, 10, 4, 16};
    unsigned op = legal_ops[
        rnd_pick(rs, (unsigned)(sizeof(legal_ops)/sizeof(legal_ops[0])))
    ];
    ...
    switch (op){
        ...
        /* ===== B. Boundary values (empty tag / overlong tag) ===== */
        case 2:  op_remove_tag_param(h); break;
        case 5:  op_long_tag(h); break;

        /* ===== C. Equivalence-class alternatives (URI scheme / host variants) ===== */
        ...
        /* ===== D. Allowed bitfield / enum / range: n/a (scheme is open token) ===== */

        /* ===== E. Encoding-validity preservation: n/a for structured addr-spec ===== */

        /* ===== F. Padding / alignment (overlong display fill) ===== */
        case 16: {
        ...
        /* ===== G. Prefix / suffix variants: n/a for structured addr-spec ===== */

        /* ===== H. Random valid mix: n/a for structured multi-field addr-spec ===== */

        default: op_canonical_min(h); break;
    }
}
...
static void mutate_one_rr_line(sip_record_route_hdr_t *h, unsigned *rs){
    ...
    /* A..H ordered table; IDs: 0=A, 3=B, 4=C, 5=C, 7=C, 16=E, 13=F, 17=F, 14=G */
    static const unsigned legal_ops[] = {
        0, 3, 4, 5, 7, 16, 13, 17, 14
    };
    unsigned op = legal_ops[
        rnd_pick(rs, (unsigned)(sizeof(legal_ops)/sizeof(legal_ops[0])))
    ];
    ...
    switch (op){
        /* ===== A. Canonical form (angle-quoted addr-spec) ===== */
        case 0:  rr_op_toggle_angle(h); break;

        /* ===== B. Boundary values (empty URI) ===== */
        case 3:  rr_op_empty_uri(h); break;

        /* ===== C. Equivalence-class alternatives (URI scheme / host forms) ===== */
        case 4:  rr_op_set_tel_uri(h); break;
        case 5:  rr_op_set_sips(h); break;
        case 7:  rr_op_ipv6(h); break;

        /* ===== D. Allowed bitfield / enum / range: n/a (open URI scheme) ===== */

        /* ===== E. Encoding-validity preservation (case-variant of lr param) ===== */
        case 16: rr_op_randomize_case_lr(h); break;

        /* ===== F. Padding / alignment (overlong params / overlong URI) ===== */
        case 13: rr_op_long_params(h); break;
        case 17: rr_op_overlong_uri(h); break;

        /* ===== G. Prefix / suffix variants (header injection via CRLF) ===== */
        case 14: rr_op_inject_header(h); break;

        /* ===== H. Random valid mix: n/a for structured addr-spec ===== */
...
static void mutate_one_timestamp(sip_timestamp_hdr_t *h, unsigned *rs){
    ...
    switch (rnd_pick(rs, 24)){
        /* ===== A. Canonical form (typical zero / fractional decimal) ===== */
        case 1:  ts_op_zero(h); break;
        case 3:  ts_op_many_decimals(h); break;

        /* ===== B. Boundary values (empty, big, negative) ===== */
        case 0:  ts_op_empty_value(h); break;
        case 2:  ts_op_bigint(h); break;
        case 6:  ts_op_negative(h); break;

        /* ===== C. Equivalence-class alternatives (numeric format variants + delay subfield) ===== */
        ...
        /* ===== D. Allowed bitfield / enum / range: n/a for numeric ===== */

        /* ===== E. Encoding-validity preservation (non-numeric / whitespace stress) ===== */
        case 8:  ts_op_alpha_value(h); break;
        case 10: ts_op_value_ws(h); break;

        /* ===== F. Padding / alignment (overlong fill) ===== */
        case 19: ts_op_overlong_value(h); break;
        case 20: ts_op_overlong_delay(h); break;

        /* ===== G. Prefix / suffix variants (CRLF / header injection) ===== */
        case 18: ts_op_inject_header(h); break;

        /* ===== H. Random valid mix (random alnum value / delay) ===== */
...
static void mutate_one_via(sip_via_hdr_t *h, unsigned *rs){
    ...
    switch (rnd_pick(rs, 34)){
        /* ===== A. Canonical form (SIP/2.0/UDP typical) ===== */
        case 0: via_op_proto_udp(h); break;

        /* ===== B. Boundary values (empty host, port 0 / over-max) ===== */
        case 16: via_op_by_empty(h); break;
        case 14: via_op_by_port0(h); break;
        case 15: via_op_by_port65536(h); break;

        /* ===== C. Equivalence-class alternatives (sent-by host forms) ===== */
        ...
        /* ===== D. Allowed bitfield / enum / range (transport enum members) ===== */
        ...
        /* ===== E. Encoding-validity preservation (separator / spacing stress) ===== */
        ...
        /* ===== H. Random valid mix (random branch token + param variants) ===== */
...
static void mutate_one_invite_uri(char *dst, size_t cap, uint32_t *seed) {
    ...
    switch (op) {
        ...
        /* ===== E. Encoding-validity preservation (percent-encoded user) ===== */
        case 6:  op7_percent_user(dst, cap); break;

        /* ===== F. Padding / alignment (overlong host fill) ===== */
        default: opX_build_long_host(dst, cap, seed); break;

        /* ===== G. Prefix / suffix variants (whitespace injection) ===== */
        case 10: op11_ws_injection(dst, cap, seed); break;

        /* ===== H. Random valid mix: n/a for structured URI ===== */
    }
}
...
static void mutate_one_body(sip_packet_t *p, uint32_t *seed){
  ...
  switch (op){
    /* ===== A. Canonical form (minimal SDP) ===== */
    case 0: {
      gen_min_sdp(buf, sizeof(buf), "203.0.113.10");
      write_body_and_maybe_len(p, buf, "application","sdp",";charset=utf-8", -1);
    } break;

    /* ===== B. Boundary values (empty / mismatched content-length) ===== */
    case 9: {
      b.body[0]='\0';
      clear_ct(b.ct);
      set_cl_len(b.cl, 0);
    ...
    /* ===== C. Equivalence-class alternatives (alternate body content-types) ===== */
    case 4: { /* text/plain */
    ...
    /* ===== D. Allowed bitfield / enum / range: n/a for free-form body ===== */

    /* ===== E. Encoding-validity preservation (UTF-8 / control bytes / dup lines) ===== */
    case 3: {
    ...
    /* ===== F. Padding / alignment (big SDP fill) ===== */
    case 1: {
      gen_big_sdp(buf, sizeof(buf));
      write_body_and_maybe_len(p, buf, "application","sdp","", -1);
    } break;

    /* ===== G. Prefix / suffix variants (leading CRLF injection) ===== */
    default: {
      snprintf(buf,sizeof(buf), "\r\n\r\nv=0\r\ns=-\r\nc=IN IP4 0.0.0.0\r\nm=audio 0 RTP/AVP 0\r\n");
      write_body_and_maybe_len(p, buf, "application","sdp","", -1);
    } break;

    /* ===== H. Random valid mix: covered by case 4 (randomised text body) ===== */
  }
...
static void mutate_one_call_id(sip_call_id_hdr_t *h){
    ...
    /* ===== B. Boundary values (empty / maxlen) ===== */
    case 4: {
        set_value(h, "");
        break;
    }
    ...
    /* ===== C. Equivalence-class alternatives (compact name / port / list) ===== */
    case 2: {
        gen_token2(tkn, sizeof(tkn));
        gen_host(host, sizeof(host));
    ...
    /* ===== D. Allowed bitfield / enum / range: n/a for token string ===== */

    /* ===== E. Encoding-validity preservation (UTF-8 / bad chars) ===== */
    case 7: {
    ...
    /* ===== F. Padding / alignment: covered by case 6 (maxlen fill) ===== */

    /* ===== G. Prefix / suffix variants (whitespace / bad CRLF / bad colon_space) ===== */
    case 5: {
        gen_token2(tkn, sizeof(tkn));
        gen_host(host, sizeof(host));
        snprintf(buf, sizeof(buf), "  %s \t @ \t %s  ", tkn, host);
    ...
    /* ===== H. Random valid mix (random token / case variants) ===== */
    case 0: {
        gen_token2(tkn, sizeof(tkn));
        set_value(h, tkn);
        break;
