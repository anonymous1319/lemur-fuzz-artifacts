...
typedef smtp_packet_t proto_packet_t;

/* parser / reassembler prototypes */
extern size_t parse_smtp_msg(const uint8_t *buf, uint32_t buf_len,
                                 proto_packet_t *out_packets, uint32_t max_count);

extern int reassemble_smtp_msgs(const proto_packet_t *packets, uint32_t num_packets,
 ...
 */
extern void free_smtp_packets(proto_packet_t *packets, uint32_t num_packets) __attribute__((weak));

static inline size_t proto_parse(const uint8_t *buf, uint32_t len,
                                 proto_packet_t *out_packets, uint32_t max_count) {
  return parse_smtp_msg(buf, len, out_packets, max_count);
}
...
static inline int proto_reassemble(const proto_packet_t *packets, uint32_t num_packets,
  ...
  return reassemble_smtp_msgs(packets, num_packets, output_buf, out_len_out);
}
...
static inline void proto_packets_cleanup(proto_packet_t *packets, uint32_t num_packets, uint32_t max_count) {
  (void)max_count;
  if (free_smtp_packets) free_smtp_packets(packets, num_packets);
  memset(packets, 0, (size_t)max_count * sizeof(proto_packet_t));
}
